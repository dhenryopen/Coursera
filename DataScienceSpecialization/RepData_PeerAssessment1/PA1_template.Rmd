---
title: "Reproducible Research: [Peer Assessment 1](https://class.coursera.org/repdata-034/human_grading/view/courses/975147/assessments/3/submissions)"
output: 
  html_document:
    keep_md: true
---
...
* The total steps taken across all days is **`r total_steps_all_days`**
* The mean steps taken across all days is **`r mean_steps_all_days`**
* The median steps taken across all days is **`r median_steps_all_days`**
...
```
* Interval **`r average_daily_activity_pattern[max_steps_row,]$interval`** has the maximum number of steps: **`r average_daily_activity_pattern[max_steps_row,]$mean_activity`**

## How many missing values are in the data set?

```{r chunk7, results="hide"}
sum(is.na(activity_df_all))
```

**`r sum(is.na(activity_df_all))`** rows contain NA
...
* With NAs fixed, the mean steps taken across all days is **`r mean_steps_fixed`** (higher)
* With NAs fixed, the median steps taken across all days is **`r median_steps_fixed`** (no difference)
